...
class BaseQRCodeReaderVC: BaseVC, AVCaptureMetadataOutputObjectsDelegate {
    ...
    func captureOutput(captureOutput: AVCaptureOutput!, didOutputMetadataObjects metadataObjects: [AnyObject]!, fromConnection connection: AVCaptureConnection!) {
        ...
        if metadataObj.type == AVMetadataObjectTypeQRCode {
            // If the found metadata is equal to the QR code metadata then do stuff!
            let barCodeObject = videoPreviewLayer?.transformedMetadataObjectForMetadataObject(metadataObj as AVMetadataMachineReadableCodeObject) as! AVMetadataMachineReadableCodeObject
            qrCodeFrameView?.frame = barCodeObject.bounds;
            
            if metadataObj.stringValue != nil {
                scanWasSucessful(metadataObj)
            }
        }
    }
    
    // Override this method to customize what to do with qr code data.
    func scanWasSucessful(metadataObj: AVMetadataMachineReadableCodeObject) {
